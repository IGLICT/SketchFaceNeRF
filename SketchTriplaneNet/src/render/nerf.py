...
class _RenderWrapper(torch.nn.Module):
    ...
    def query_voxel(self, sb, device, render=True):
        out, rgb_coarse = self.renderer.query_voxel(self.net, sb=sb, device=device, render=render)
        return out, rgb_coarse
...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            ...
            if sb > 0:
                ...
                eval_batch_dim = 0
            #print("eval_batch_dim:", eval_batch_dim)
            ...
            if use_viewdirs:
                ...
                for pnts in split_points:
                    ...
                    print("PyTorch ms:", (end-start)/rerun)
                    exit()
                    """
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            #print("z_coarse:", z_coarse.shape)
            coarse_composite = self.composite(
                model, rays, z_coarse, coarse=True, sb=superbatch_size,
            )
            #torch.cuda.synchronize()
            #end = time.time()
            #print("time composite: ", end-start)
            ...
            if self.using_fine:
                all_samps = [z_coarse]
                #print("n_fine:", self.n_fine)
                #print("n_fine_depth:", self.n_fine_depth)
