...
import warnings
from .networks_maskGAN import weights_init, GlobalGenerator_adain
import time
...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        self.mlp_coarse = make_mlp(conf["mlp_coarse"], d_in, d_latent * 2, d_out=d_out)
        self.mlp_fine = make_mlp(
            conf["mlp_fine"], d_in, d_latent * 2, d_out=d_out, allow_empty=True
        )
        ...
        batch = 200000
        self.index = torch.cat((torch.arange(0,batch,2), torch.arange(1,batch+1,2)))
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False):
        ...
        with profiler.record_function("model_inference"):
            #print(xyz)
            SB, B, _ = xyz.shape
            #print("xyz max:", torch.max(xyz))
            #print("xyz min:", torch.min(xyz))
            ...
            if self.use_encoder:
                ...
                if self.stop_encoder_grad:
                    ...
                    latent_flip = latent_flip.detach()
                latent = latent.transpose(1, 2).reshape(
                    -1, self.latent_size
                )  # (SB * NS * B, latent)
                latent_flip = latent_flip.transpose(1, 2).reshape(
                    -1, self.latent_size
                )  # (SB * NS * B, latent)
                ...
                if self.d_in == 0:
                    ...
                    mlp_input = torch.cat((latent, latent_flip, z_feature), dim=-1)
