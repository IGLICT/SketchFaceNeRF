...
from util import combine_interleaved
...
class ResnetFC_fea(nn.Module):
    def __init__(
        ...
        self.lin_out_1 = nn.Linear(d_hidden, 32)
        nn.init.constant_(self.lin_out_1.bias, 0.0)
        nn.init.kaiming_normal_(self.lin_out_1.weight, a=0, mode="fan_in")
        #self.activation_1 = nn.ReLU()
        self.lin_out_2 = nn.Linear(32, d_out)
        nn.init.constant_(self.lin_out_2.bias, 0.0)
        nn.init.kaiming_normal_(self.lin_out_2.weight, a=0, mode="fan_in")
    ...
    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            for blkid in range(self.n_blocks):
                ...
                x = self.blocks[blkid](x)
            #out = self.lin_out(self.activation(x))
            out = self.lin_out_1(self.activation(x))
            out = self.lin_out_2(self.activation(out))
            return out
